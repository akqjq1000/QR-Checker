...
# 도메인을 서브도메인·등록 도메인·suffix로 정확하게 분리하는 도구
...
def _parse_http_url(url: str) -> tuple[str, SplitResult, str, int]:
    ...
    # 포트가 URL에 직접 적혀 있지 않으면 schema.py 기준에 따라 -1을 사용
    port = explicit_port if explicit_port is not None else -1

    # 도메인 비교가 일정하도록 소문자로 바꾸고 마지막의 불필요한 점을 제거
...
def _is_ip_address(hostname: str) -> bool:
    ...
    # ipaddress가 정상적으로 해석하면 IP이고, 실패하면 일반 도메인
    try:
...
def _calculate_entropy(text: str) -> float:
    ...
    # 각 문자의 등장 확률 p에 대해 -Σ(p * log2(p))를 계산
    return -sum(
...
def extract_features(url: str) -> FeatureVector:
    ...
    features = FeatureVector(
        ...
        # --- 개수 관련 피처 ---
        # 서브도메인이 a.b라면 점으로 나눈 결과가 2개이므로 2를 반환
        count_sub_domain=len(subdomain.split(".")) if subdomain else 0,

        # path 안의 '/' 개수를 경로 깊이 기준으로 사용

...
class RAGEngine:
    ...
    def perform_web_search(self, query: str) -> str:
        if query in self.searched_queries:
            return (
                "System Action: 이미 검색했던 내용이야. 기존 정보를 바탕으로 답변해 줘."
            )
        ...
        self.search_count += 1
        print(
            f"[Web Search] '{query}' 검색 중... ({self.search_count}/{self.search_budget})"
        )
    ...
    def _generate_final_guide(self, session_id: str, context_prompt: str) -> dict:
        ...
        user_input = f"상황: {context_prompt}\n\n[보안 지침 문서]\n{retrieved_context}"
        ...
        try:
            ...
            clean_json_str = (
                assistant_response.replace("```json", "").replace("```", "").strip()
            )
    ...
    if response1["status"] == "chat_required":
        ...
        if response2["status"] == "chat_required":
            ...
            response3 = engine.chat(
                session_id=session_id, user_message="1. 로그인/비밀번호 입력"
            )

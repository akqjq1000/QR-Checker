...
class RAGEngine:
    def __init__(self):
        ...
        self.search_budget = 3       # 대화당 최대 검색 횟수
        self.search_count = 0        # 현재 검색 횟수 카운터
        self.searched_queries = set() # 중복 검색어 방지용 Set

    # ---------------------------------------------------------
    # [사용자 DB 연동 기능]
    # ---------------------------------------------------------
    def _init_db(self):
        """SQLite DB 테이블 생성 (없으면 생성)"""
    ...
    # ---------------------------------------------------------
    # [웹 검색 로직 (Search Budget 반영)]
    # ---------------------------------------------------------
    def perform_web_search(self, query: str) -> str:
        """웹 검색 수행 (중복 쿼리 방지 및 횟수 제한)"""
        ...
        try:
            with DDGS() as ddgs:
                # max_results로 가져올 검색 결과 개수를 조절할 수 있어
    ...
    def init_scan(self, url: str, detection_result: DetectionResult) -> dict:
        ...
        # ML 판단이 악성이거나, DB에 기록되어 있으면 위험으로 간주
        ...
        # 위험 판정 시 DB에 등록 (중복 처리됨)
        if not is_db_bad:
            self.register_malicious_url(url)
            
        session_id = str(uuid.uuid4())
        
        # 새 세션 시작 시 검색 횟수/기록 초기화
        self.search_count = 0
    ...
    def _generate_final_guide(self, session_id: str, context_prompt: str) -> dict:
        system_instructions = (
            "너는 전문적인 보안 컨설턴트야. 제공된 [보안 지침 문서]와 필요시 [웹 검색 결과]를 종합해서 "
            "사용자의 상황에 맞는 판단 사유와 대응 지침을 작성해 줘. "
            "가장 관련 깊은 문서의 이름이나 웹 검색 출처를 'source'에 명시해야 해. "
            "반드시 마크다운이나 추가 설명 없이 아래 JSON 형식으로만 정확하게 답변해.\n"
            "{\n"
            "  \"reason\": \"위험/안전 판단 상세 사유\",\n"
            "  \"countermeasures\": [\"대응 지침 1\", \"대응 지침 2\"],\n"
            "  \"source\": \"참고한 핵심 출처 (예: KISA 문서, DuckDuckGo 검색 등)\"\n"
            "}"
        )
        ...
        # 🔧 1. GPT에게 알려줄 함수 스펙(도구) 정의
        tools = [
            {
                "type": "function",
                "function": {
                    "name": "perform_web_search",
                    "description": "제공된 내부 보안 지침 문서만으로 부족하거나, 최신 악성 URL 및 피싱 수법에 대한 추가 정보가 필요할 때 웹 검색을 수행합니다.",
                    "parameters": {
                        "type": "object",
                        "properties": {
                            "query": {
                                "type": "string",
                                "description": "검색할 키워드 (예: 최신 피싱 수법, 특정 URL 악성 여부 등)"
                            }
                        },
                        "required": ["query"]
                    }
        ...
        try:
            # 🔧 2. 1차 API 호출 (GPT가 도구를 쓸지 말지 스스로 판단)
            response = self.client.chat.completions.create(
                model="gpt-4o", # gpt-3.5-turbo 등 사용 가능한 모델 입력
                messages=messages,
                tools=tools,
                tool_choice="auto" 
            )
            ...
            # 🔧 3. GPT가 웹 검색 도구를 사용하겠다고 판단한 경우
            if tool_calls:
                messages.append(response_message) # GPT의 도구 호출 기록을 대화 내역에 추가
                ...
                for tool_call in tool_calls:
                    ...
                    if function_name == "perform_web_search":
                        # 실제 우리 파이썬 클래스의 웹 검색 함수 실행
                        function_response = self.perform_web_search(
                            query=function_args.get("query")
                        )
                        
                        # 검색 결과를 messages 리스트에 추가하여 GPT에게 전달
                        messages.append({
                ...
                # 🔧 4. 검색 결과를 바탕으로 2차 API 호출 (최종 답변 생성)
            ...
            # 스키마(AnalysisResult) 처리를 위해 reason에 source 이어 붙이기
